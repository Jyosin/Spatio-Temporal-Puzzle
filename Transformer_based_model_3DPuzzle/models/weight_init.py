...
def kaiming_init_(tensor,
    ...
    if distribution == 'uniform': nn.init.kaiming_uniform_(tensor, a=a, mode=mode, nonlinearity=nonlinearity)
    else: nn.init.kaiming_normal_(tensor, a=a, mode=mode, nonlinearity=nonlinearity)

@torch.no_grad()
def weight_compile(state_dict,mode='21k'):
	if mode=='21k':
		...
		for old_key in old_state_dict_keys:
			new_key=old_key
			if 'patch_embed.proj' in old_key: new_key=old_key.replace('patch_embed.proj','patch_embed.projection')
			elif 'blocks' in old_key:
				new_key=old_key.replace('blocks','transformer_layers.layers')

				new_key=new_key.replace('norm1','attentions.0.norm')
				new_key=new_key.replace('norm2','ffns.0.norm')

				new_key=new_key.replace('attn','attentions.0.attn')
...
@torch.no_grad()
def init_from_vit_pretrain(module,
						    pretrained,
						    conv_type, 
						    attention_type, 
						    copy_strategy,
	...
	if isinstance(pretrained, str):
		...
		mode=os.path.basename(pretrained).split('.')[0].split('-')[1]

		state_dict=weight_compile(state_dict,mode)
