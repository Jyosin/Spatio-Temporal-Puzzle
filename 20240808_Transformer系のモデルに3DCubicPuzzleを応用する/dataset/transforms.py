...
class CubicPuzzle3D(object):
	def __init__(self,img_size,n_frames,n_grid=(2,2,4),grayscale_prob=0.5,jitter_size=(80,80,16),crop_mode='center'):
		...
		if crop_mode.upper()=='CENTER': resize_fn=transforms.CenterCrop(img_size)
		else : resize_fn=transforms.RandomResizedCrop(img_size,scale=(1,1),ratio=(1,1))
	...
	def __call__(self,x:torch.Tensor):
		# Transform
		if random.uniform(0,1)>self.grayscale_prob: x=self.grayscale(x)
		x=self.transforms(x)
		T,C,H,W=x.size()
		
		# Generate Spatio-Temporal Cells
		h,w,t=self.n_grid
...
#  ----------------------  Jigsaw  ----------------------------
#  ------------------------------------------------------------
class MaskingJigsaw(object):
	def __init__(self,temporal_piece_size,spatial_piece_size,temporal_jigsaw_ratio=0.5,spatial_jigsaw_ratio=0.5):
		self.temporal_piece_size=temporal_piece_size
		self.spatial_piece_size=spatial_piece_size
		self.temporal_jigsaw_ratio=temporal_jigsaw_ratio
		self.spatial_jigsaw_ratio=spatial_jigsaw_ratio

	def __call__(self,tensor:torch.Tensor):
		if self.temporal_piece_size!=0: tensor,temporal_indices=self.temporal_jigsaw(tensor)
		else: temporal_indices=None

		if self.spatial_piece_size!=0: tensor,spatial_indices=self.spatial_jigsaw(tensor)
		else: spatial_indices=None

		return tensor,temporal_indices,spatial_indices

	def temporal_jigsaw(self,tensor:torch.Tensor):
		# T × C × H × W
		t,c,h,w=tensor.size()
		assert t%self.temporal_piece_size==0,f"temporal size {t} must be dividable by piece size {self.temporal_piece_size}"
		n_piece=t//self.temporal_piece_size
		n_mask=np.ceil(n_piece*self.temporal_jigsaw_ratio).astype(int)
		
		tensor=rearrange(tensor,'(p n) C H W -> p n C H W',n=self.temporal_piece_size,p=n_piece)
		
		piece_indices=np.arange(n_piece)
		mask_indices=np.random.choice(n_piece,n_mask,replace=False)
		
		tensor[mask_indices]=0
		piece_indices=np.setdiff1d(piece_indices,mask_indices)
		
		tensor=rearrange(tensor,'p n C H W -> (p n) C H W')
	
		return tensor,torch.from_numpy(piece_indices).long()

	def spatial_jigsaw(self,tensor:torch.Tensor):
		# T × C × H × W
		t,c,h,w=tensor.size()
		assert h%self.spatial_piece_size==0 and w%self.spatial_piece_size==0,f"spatial size ({h},{w}) must be dividable by piece size {self.spatial_piece_size}"
		n_piece_h=(h//self.spatial_piece_size)
		n_piece_w=(w//self.spatial_piece_size)
		n_mask=np.ceil(n_piece_h*n_piece_w*self.spatial_jigsaw_ratio).astype(int)
		
		tensor=rearrange(tensor,'T C (nh h) (nw w) -> T C (nh nw) h w',nh=n_piece_w,nw=n_piece_w,h=self.spatial_piece_size,w=self.spatial_piece_size)
		
		piece_indices=np.arange(n_piece_h*n_piece_w)
		mask_indices=np.random.choice(n_piece_h*n_piece_w,n_mask,replace=False)
		
		tensor[:,:,mask_indices]=0
		piece_indices=np.setdiff1d(piece_indices,mask_indices)
		
		tensor=rearrange(tensor,'T C (nh nw) h w -> T C (nh h) (nw w)',nh=n_piece_w,nw=n_piece_w)
		
		return tensor,torch.from_numpy(piece_indices).long()
	
class ShuffleJigsaw(object):
	def __init__(self,temporal_piece_size,spatial_piece_size,temporal_jigsaw_ratio=0.5,spatial_jigsaw_ratio=0.5):
		self.temporal_piece_size=temporal_piece_size
		self.spatial_piece_size=spatial_piece_size
		self.temporal_jigsaw_ratio=temporal_jigsaw_ratio
	...
	def temporal_jigsaw(self,tensor:torch.Tensor):
		...
		assert t%self.temporal_piece_size==0,f"temporal size {t} must be dividable by piece size {self.temporal_piece_size}"
		n_piece=t//self.temporal_piece_size
		n_shuffle=np.ceil(n_piece*self.temporal_jigsaw_ratio).astype(int)
		
		tensor=rearrange(tensor,'(p n) C H W -> p n C H W',n=self.temporal_piece_size,p=n_piece)
		
		piece_indices=np.arange(n_piece)
		change_indices=np.random.choice(n_piece,n_shuffle*2,replace=False)
		
		for i in range(n_shuffle):
			index_a=change_indices[i*2]
			index_b=change_indices[i*2+1]
			tensor_a=tensor[index_a].clone()
			tensor_b=tensor[index_b].clone()
			tensor[index_a]=tensor_b
			tensor[index_b]=tensor_a
			
			val_a=piece_indices[index_a].copy()
			val_b=piece_indices[index_b].copy()
			piece_indices[index_a]=val_b
			piece_indices[index_b]=val_a
	...
	def spatial_jigsaw(self,tensor:torch.Tensor):
		...
		n_piece_h=(h//self.spatial_piece_size)
		n_piece_w=(w//self.spatial_piece_size)
		n_shuffle=np.ceil(n_piece_h*n_piece_w*self.spatial_jigsaw_ratio).astype(int)

		tensor=rearrange(tensor,'T C (nh h) (nw w) -> T C (nh nw) h w',nh=n_piece_w,nw=n_piece_w,h=self.spatial_piece_size,w=self.spatial_piece_size)
		
		piece_indices=np.arange(n_piece_h*n_piece_w)
		change_indices=np.random.choice(n_piece_h*n_piece_w,n_shuffle*2,replace=False)
...
class PatternedShuffleJigsaw(object):
	def __init__(self,temporal_piece_size,spatial_piece_size,n_temporal_pattern=3,n_spatial_pattern=32):
		self.temporal_piece_size=temporal_piece_size
		self.spatial_piece_size=spatial_piece_size
		self.n_temporal_pattern=n_temporal_pattern
		self.n_spatial_pattern=n_spatial_pattern
		self.temporal_patterns=None
		self.spatial_patterns=None
	...
	def temporal_jigsaw(self,tensor:torch.Tensor):
		...
		if self.temporal_patterns is None:
			assert n_piece>=self.n_temporal_pattern,f"n_temporal_pattern {self.n_temporal_pattern} must be smaller than total piece number {n_piece}"
			self.temporal_patterns=[]
			for _ in range(self.n_temporal_pattern): self.temporal_patterns.append(np.random.permutation(n_piece))

		pattern_idx=np.random.choice(self.n_temporal_pattern)
		shuffle_pattern=self.temporal_patterns[pattern_idx]

		tensor=rearrange(tensor,'(p n) C H W -> p n C H W',n=self.temporal_piece_size,p=n_piece)
		
		tensor=tensor[shuffle_pattern]
		
		tensor=rearrange(tensor,'p n C H W -> (p n) C H W')
	
		return tensor,torch.Tensor([pattern_idx]).long()
	...
	def spatial_jigsaw(self,tensor:torch.Tensor):
		...
		if self.spatial_patterns is None:
			assert n_piece_h*n_piece_w>=self.n_spatial_pattern,f"n_spatial_pattern {self.n_spatial_pattern} must be smaller than total piece number {n_piece_h*n_piece_w}"
			self.spatial_patterns=[]
			for _ in range(self.n_spatial_pattern): self.spatial_patterns.append(np.random.permutation(n_piece_h*n_piece_w))

		pattern_idx=np.random.choice(self.n_spatial_pattern)
		shuffle_pattern=self.spatial_patterns[pattern_idx]
...
def transforms_train(img_size=224,
	...
	ratio = tuple(ratio or (1.0, 1.0))  # default imagenet ratio range
	primary_tfl = [
...
def main():
	...
	print(os.getcwd())
	from dataset.dataset import DecordInit
	...
	dec=DecordInit()
	reader=dec(path)
	frame_indice=np.arange(len(reader))[::len(reader)//n_frame][:n_frame]
	video=reader.get_batch(frame_indice).asnumpy()

	transform=transforms.CenterCrop(224)
	video=transform(torch.from_numpy(video).permute(0,3,1,2))
	
	for i,v in video: imsave(v,f"./before/{i:02d}.png")

	jigsaw=Jigsaw(temporal_piece_size=0,spatial_piece_size=56)
	video,temp_indices,spat_indices=jigsaw(video)

	for i,v in video: imsave(v,f"./after/{i:02d}.png")
	print(spat_indices)
